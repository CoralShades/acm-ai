...
def resolve_conflict(match):
    ...
    if 'epic-19' in head and 'epic-18' in theirs:
        ...
        theirs_updated = theirs.replace(
            '# 1/4 stories complete', '# 6/6 stories complete'
        ).replace(
            'epic-18: in-progress', 'epic-18: done'
        ).replace(
            ...
            'pending live E2E validation', 'improvements done'
        )
        head_fixed = head.replace(
            '# 1/1 stories complete', '# 1/1 stories complete'
        )
