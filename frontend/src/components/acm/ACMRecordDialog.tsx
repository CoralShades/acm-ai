...
const acmRecordSchema = z.object({
  ...
  building_year: z.coerce.number().int().optional().or(z.literal('')),
  building_construction: z.string().optional(),
  room_id: z.string().optional(),
  room_name: z.string().optional(),
  room_area: z.coerce.number().optional().or(z.literal('')),
  ...
  page_number: z.coerce.number().int().optional().or(z.literal('')),
})
  ...
  const onSubmit = async (data: ACMRecordFormData) => {
    ...
    if (mode === 'create') {
      const createData: ACMRecordCreateRequest = {
        ...
        result: data.result,
        ...(cleanData.school_code && { school_code: cleanData.school_code as string }),
        ...(cleanData.building_name && { building_name: cleanData.building_name as string }),
        ...(cleanData.building_year && { building_year: cleanData.building_year as number }),
        ...(cleanData.building_construction && { building_construction: cleanData.building_construction as string }),
        ...(cleanData.room_id && { room_id: cleanData.room_id as string }),
        ...(cleanData.room_name && { room_name: cleanData.room_name as string }),
        ...(cleanData.room_area && { room_area: cleanData.room_area as number }),
        ...(cleanData.area_type && { area_type: cleanData.area_type as string }),
        ...(cleanData.extent && { extent: cleanData.extent as string }),
        ...(cleanData.location && { location: cleanData.location as string }),
        ...(cleanData.friable && { friable: cleanData.friable as string }),
        ...(cleanData.material_condition && { material_condition: cleanData.material_condition as string }),
        ...(cleanData.risk_status && { risk_status: cleanData.risk_status as string }),
        ...(cleanData.page_number && { page_number: cleanData.page_number as number }),
      }
      await createRecord.mutateAsync(createData)
    } else if (record) {
      const updateData: ACMRecordUpdateRequest = cleanData as ACMRecordUpdateRequest
