...
BAR_COLUMNS = [
    ...
    'Building Name', 'Building Type', 'Building Address', 'Suburb', 'Postcode',
    'Owned or Leased', 'Building Unique ID', 'Frequency of use', 'Public Access?',
    'Date of Inspection', 'Estimated Year Built', 'Est. Building Size (m2)',
    'Number of Levels', 'Construction Type', 'Roof Type', 'Internal / External',
    'Level', 'Room or Area', 'Location in Room', 'Specific Item/ACM Name',
    'Friability of material', 'ACM Product Group', 'ACM Product Type',
    'NATA Endorsed Sample number (if available)', 'Sample Result',
    'Identifying Hygiene or Consulting Company', 'Condition', 'Disturbance Potential',
    'Quantity', 'Labelled', 'Label Details', 'Hygienist Recommendations',
    'Additional Comments', 'PSB Supplied ACM ID', 'Assumed Removed?',
    'Date of Removal', 'Quantity Removed', 'Asbestos Removal Notification No',
...
def classify_page(text: str) -> str:
    ...
    if 'division 5 asbestos assessment' in text_lower and 'client no' in text_lower:
        ...
        return 'executive'
    elif 'asbestos register' in text_lower and ('area' in text_lower or 'level' in text_lower):
        return 'register'
...
def extract_metadata_from_text(pages: dict) -> dict:
    ...
    addr_match = re.search(r'(\d+\s+[\w]+\s+(?:Road|Street|Avenue|Drive|Lane|Parade|Crescent|Way))\s*\n\s*(\w+)', all_text, re.IGNORECASE)
    if addr_match:
    ...
    site_addr_match = re.search(r'Site Address\s*\n?\s*(\d+\s+[\w\s]+),\s*(\w+)', all_text, re.IGNORECASE)
    if site_addr_match:
    ...
    date_match = re.search(r'(?:Assessment|conducted)\s+(?:on\s+)?(?:the\s+)?(\d{1,2})(?:st|nd|rd|th)?\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{4})', all_text, re.IGNORECASE)
    if date_match:
        day, month, year = date_match.groups()
        month_map = {'january': 1, 'february': 2, 'march': 3, 'april': 4, 'may': 5, 'june': 6,
                     'july': 7, 'august': 8, 'september': 9, 'october': 10, 'november': 11, 'december': 12}
        metadata['inspection_date'] = datetime(int(year), month_map[month.lower()], int(day))
    
    # Fallback: look for Date of Identification in register (usually Apr-20 format)
    if not metadata['inspection_date']:
        date_match2 = re.search(r'(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)-(\d{2})', all_text)
        if date_match2:
            month_abbr, year_short = date_match2.groups()
            month_map = {'jan': 1, 'feb': 2, 'mar': 3, 'apr': 4, 'may': 5, 'jun': 6,
                        'jul': 7, 'aug': 8, 'sep': 9, 'oct': 10, 'nov': 11, 'dec': 12}
            year = 2000 + int(year_short)
            metadata['inspection_date'] = datetime(year, month_map[month_abbr.lower()], 1)
    ...
    if postcode_match:
        ...
        if metadata['suburb']:
            suburb_context = all_text[all_text.lower().find(metadata['suburb'].lower()):all_text.lower().find(metadata['suburb'].lower())+100]
...
def extract_color_note(description: str) -> Optional[str]:
    ...
    colors = ['cream', 'grey', 'gray', 'brown', 'black', 'beige', 'white', 'blue', 'green', 'orange', 'speckled']
    desc_lower = description.lower()
...
def transform_to_bar_row(metadata: dict, row: dict) -> dict:
    ...
    bar_row = {
        ...
        'Building Type': 'Police Station' if 'police' in metadata.get('site_name', '').lower() else '',
        ...
        'NATA Endorsed Sample number (if available)': normalize_sample_number(row.get('sample_number', '')),
        'Sample Result': normalize_sample_result(row.get('hazard_status', '')),
        'Identifying Hygiene or Consulting Company': metadata.get('consultant', 'Prensa Pty Ltd'),
        'Condition': 'N/A (negative)' if is_negative else row.get('condition', 'Good'),
        'Disturbance Potential': 'N/A (negative)' if is_negative else row.get('disturbance_potential', 'Low'),
        'Quantity': None if row.get('quantity', '-') == '-' else row.get('quantity'),
        'Labelled': 'YES' if row.get('labelled', '').lower() == 'yes' else 'NO',
        'Label Details': 'Labelled' if row.get('labelled', '').lower() == 'yes' else None,
        'Hygienist Recommendations': None if row.get('comments', '-') == '-' else row.get('comments'),
...
def create_bar_excel(rows: list, output_path: str):
    ...
    # Set column widths
    column_widths = {
        'A': 12, 'B': 15, 'C': 25, 'D': 25, 'E': 25, 'F': 15,
        'G': 20, 'H': 15, 'I': 10, 'J': 12, 'K': 15, 'L': 12,
        'M': 12, 'N': 15, 'O': 12, 'P': 15, 'Q': 12, 'R': 15,
        'S': 12, 'T': 15, 'U': 12, 'V': 25, 'W': 20, 'X': 20,
        'Y': 15, 'Z': 25, 'AA': 20, 'AB': 25, 'AC': 15, 'AD': 20,
        'AE': 12, 'AF': 15, 'AG': 10, 'AH': 10, 'AI': 12, 'AJ': 25,
        'AK': 20, 'AL': 15, 'AM': 15, 'AN': 15, 'AO': 15, 'AP': 25, 'AQ': 25
    }
...
if __name__ == "__main__":
    ...
    pdf_path = sys.argv[1]
    output_path = sys.argv[2] if len(sys.argv) > 2 else pdf_path.replace('.pdf', '_BAR.xlsx')
