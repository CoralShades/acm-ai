...
1. [x] B1: Trace model provisioning flow — `model_provisioning.py` → `esperanto` → LangChain ChatModel registry
2. [x] B1: Audit `GET /api/models` and `GET /api/models/defaults` — check which models are in SurrealDB `model` table
3. [x] B1: Identify why haiku/opus fail but sonnet works — likely model name format mismatch
4. [x] B2: Trace session load flow — `chatSessionStore.fetch()` → REST API → `MemorySaver` checkpointer
5. [x] B2: Check if `MemorySaver` loses state on API restart (expected) — plan durable persistence
6. [x] B3: Query `SELECT record_count FROM building_record WHERE source_id = X` — confirm NULL
7. [x] B3: Find where `record_count` should be populated — extraction vs post-extraction
8. [x] B4: Read `ExtractionStatusBanner.tsx` + `JobControls.tsx` — find extraction state logic
9. [x] B4: Trace extraction status lifecycle — when does `isExtracting` get set to false?

### Phase 2 — Implementation (Team: Frontend + Backend specialists)
10. [x] B1: Update model provisioning to support latest Anthropic models (claude-sonnet-4-5-20250514, claude-haiku-4-5-20251001, claude-opus-4-6-20250610)
11. [x] B1: Deduplicate models in SurrealDB — remove stale entries, update provider mappings
12. [x] B1: Sync available Ollama models from `GET /api/tags` — auto-register/dedup
13. [x] B2: Implement chat history loading from checkpointer when switching sessions
14. [x] B2: Auto-load last conversation (within 10 min) or create new on page refresh
15. [x] B3: Populate `record_count` via COUNT query after extraction completes
16. [x] B3: Add migration to backfill existing building records
17. [x] B4: Fix extraction status check — use `source.processing_status` not stale SSE state
18. [x] B4: Apply `/vaea-ui` design review for proper state transitions
...
25. [x] Update sprint-artifacts documentation with fix details
26. [ ] Update `docs/development/architecture.md` with chat session persistence changes
27. [x] Comment on GitHub issues (#106, #118, #119) noting fixes are implemented
