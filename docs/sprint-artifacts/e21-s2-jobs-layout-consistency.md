...
Acceptance Criteria:
- [x] Jobs dashboard (/jobs) uses same card layout patterns as Dashboard home
- [x] Job detail page (/jobs/[id]) uses same panel layout as Source Detail page
- [x] Building review (/jobs/[id]/review/buildings) uses:
...
- [x] Records review (/jobs/[id]/review/records) uses:
  - Same tab pattern as Job Detail page
  - Same AG Grid configuration as ACM Register
  - Same empty state patterns as E16-S3
- [x] All pages use consistent spacing (p-6 outer, gap-4 between sections)
- [x] All pages use VAEA design tokens from E14-S1
- [x] Dark mode works consistently across all job pages
