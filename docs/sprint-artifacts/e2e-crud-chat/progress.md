...
| T1.7 Write execute | PARTIAL | Approval sent, DB write executed, but AG-UI SSE breaks (BUG-4) |
...
4. **KNOWN** AG-UI adapter crashes on execute_write_node AIMessage response (BUG-4)
...
- Fix BUG-4: execute_write_node needs to return ToolMessage for AG-UI compatibility
